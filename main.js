...
function pickBestBMEntry(bmData, targetName) {
  ...
  const exact = data.filter((d) => {
    ...
    return (
      typeof nm === "string" && nm.toLowerCase() === targetName.toLowerCase()
    );
  });
...
function renderTeam(team) {
  if (!team) {
    ...
    teamRank.textContent =
      ...
      teamPVP.textContent =
      teamPVE.textContent =
      teamBallistics.textContent =
      teamGather.textContent =
        "—";
    emptyTeamTable();
    return;
  }
  ...
  teamMeta.textContent = `Team Members: ${
    team.SteamIDs?.length || 0
  ...
  for (const m of members) {
    const avatar =
      m?.User?.Avatar?.avatarFull ||
      m?.User?.Avatar?.avatar ||
      m?.User?.Avatar?.avatarMedium ||
      "";
    ...
    const ratingBlock = `
            ...
            ${
              mrk.PVPPerf != null
                ? `<span class="pill"><b>PVP</b>&nbsp;${mrk.PVPPerf}</span>`
                : ""
            }
            ${
              mrk.PVEPerf != null
                ? `<span class="pill"><b>PVE</b>&nbsp;${mrk.PVEPerf}</span>`
                : ""
            }
            ${
              mrk.BallisticsPerf != null
                ? `<span class="pill"><b>Ballistics</b>&nbsp;${mrk.BallisticsPerf}</span>`
                : ""
            }
            ${
              mrk.GatherPerf != null
                ? `<span class="pill"><b>Gather</b>&nbsp;${mrk.GatherPerf}</span>`
                : ""
            }
            </div>`;
    ...
    const pvpBlock = `
            <div class="group">
                ...
                <div class="row">
                    <span class="pill"><b>Arrows</b>&nbsp;${v(
                      m.ArrowsFired
                    )}</span>
                    <span class="pill"><b>Bullets</b>&nbsp;${v(
                      m.BulletsFired
                    )}</span>
                </div>
                <div class="row">
                    <span class="pill"><b>Rockets</b>&nbsp;${v(
                      m.RocketsLaunched
                    )}</span>
                    <span class="pill"><b>Explosives</b>&nbsp;${v(
                      m.ExplosivesThrown
                    )}</span>
                </div>
            </div>`;
    ...
    const pveVehBlock = `
            <div class="group">
                <div class="row">
                    <span class="pill"><b>PVE Kills</b>&nbsp;${v(
                      m.PVEKills
                    )}</span>
                    <span class="pill"><b>NPC Kills</b>&nbsp;${v(
                      m.NPCKills
                    )}</span>
                </div>
                <div class="row">
                    <span class="pill"><b>HeliHits</b>&nbsp;${v(
                      m.HeliHits
                    )}</span>
                    <span class="pill"><b>HeliKills</b>&nbsp;${v(
                      m.HeliKills
                    )}</span>
                </div>
                <div class="row">
                    <span class="pill"><b>APCHits</b>&nbsp;${v(
                      m.APCHits
                    )}</span>
                    <span class="pill"><b>APCKills</b>&nbsp;${v(
                      m.APCKills
                    )}</span>
                </div>
            </div>`;
    ...
    const gatherBlock = `
            <div class="group">
                ...
                <span class="pill sulfur"><b>Sulfur</b>&nbsp;${v(
                  m.Sulfur
                )}</span>
            </div>`;
        ...
        if (!bm) {
          ...
          if (playedEl)
            playedEl.textContent =
              bm.timePlayed != null ? fmtTime(bm.timePlayed) : "—";
        }
      } catch (e) {
...
async function fetchSteamAndTeam() {
  ...
  try {
    const res = await fetch(buildPlayerUrl(name), {
    ...
    if (!steamId) {
      setStatus("No SteamID found for that name on this server.", true);
      return;
    }
    showSteam(steamId);
    setStatus("Fetching team…");

    const tRes = await fetch(buildTeamUrl(steamId), {
      headers: { Accept: "application/json" },
    });
    if (!tRes.ok) throw new Error(`TEAM HTTP ${tRes.status}`);
    const tData = await tRes.json();
    const team = Array.isArray(tData?.leaderboard)
      ? tData.leaderboard[0]
      : null;
    renderTeam(team);
    setStatus("");
